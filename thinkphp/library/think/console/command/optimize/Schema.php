...
class Schema extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        if ($input->hasOption('module')) {
            ...
            // 读取模型
            $list = scandir(APP_PATH . $module . DS . 'common');
            $app  = App::$namespace;
            foreach ($list as $file) {
                if (0 === strpos($file, '.')) {
                    continue;
                }
                $class = '\\' . $app . '\\' . $module . '\\common\\' . pathinfo($file, PATHINFO_FILENAME);
            ...
            $app  = App::$namespace;
            $list = scandir(APP_PATH . 'common');
            foreach ($list as $file) {
                if (0 === strpos($file, '.')) {
                    continue;
                }
                $class = '\\' . $app . '\\common\\' . pathinfo($file, PATHINFO_FILENAME);
                $this->buildModelSchema($class);
            }
            $output->writeln('<info>Succeed!</info>');
            return;
